...
(function() {
  ...
  function updateLoadProgressIndicator() {
    const { done, total, itemsLoaded, complete } = loadProgress;
  ...
  async function getMarketplaceUpstreamFingerprint() {
    try {
      const response = await fetch(`${MARKETPLACE_API}?page=1`, { cache: 'no-store' });
  ...
  function saveMarketplaceCacheData(items, done, complete, sourceFingerprint = null) {
    ...
    idbSet(MARKETPLACE_IDB_KEY, cacheData).then(ok => {
      if (ok) {
  ...
  async function loadDownloadDatabase() {
    ...
    for (let attempt = 1; attempt <= DOWNLOAD_DB_RETRY_ATTEMPTS; attempt += 1) {
      ...
      try {
        ...
        console.log(`Loaded: ${map.size} Keys`);
        return downloadDb;
      } catch (error) {
        lastError = error;
      } finally {
    ...
    console.error(`Failed to fetch ${DOWNLOAD_DB_URL} after ${DOWNLOAD_DB_RETRY_ATTEMPTS} attempts`, lastError);
    downloadDb = new Map();
    downloadDbLoaded = true;
    throw lastError || new Error(`Failed to fetch ${DOWNLOAD_DB_URL}`);
  }
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
      // Show loading message briefly while the first batch loads
      setLoaderMainText('Loading');
